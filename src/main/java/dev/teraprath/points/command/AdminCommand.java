...
public class AdminCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (args.length > 2) {
            ...
            final String prefix = cfg.getString("language.prefix") + "";
            String message;
            if (target != null && amount >= 0) {
                switch (args[1].toLowerCase()) {
                    case "set":
                        PointsAPI.setPoints(target, amount);
                        message = prefix + cfg.getString("language.player_points_set");
                        sender.sendMessage(ChatColor.translateAlternateColorCodes('&', message.replace("%amount%", amount + "").replace("%player%", target.getName())));
                        break;
                    case "add":
                        PointsAPI.addPoints(target, amount);
                        message = prefix + cfg.getString("language.player_points_added");
                        sender.sendMessage(ChatColor.translateAlternateColorCodes('&', message.replace("%amount%", amount + "").replace("%player%", target.getName())));
                        break;
                    case "remove":
                        PointsAPI.removePoints(target, amount);
                        message = prefix + cfg.getString("language.player_points_removed");
                        sender.sendMessage(ChatColor.translateAlternateColorCodes('&', message.replace("%amount%", amount + "").replace("%player%", target.getName())));
                        break;
                    default:
                        String wrongUsage = prefix + cfg.getString("language.wrong_usage");
                        sender.sendMessage(ChatColor.translateAlternateColorCodes('&', wrongUsage.replace("%usage%", "/apoints <player> [set|remove|add] <amount>")));
                        break;
                }
            } else {
                wrongUsage(sender);
